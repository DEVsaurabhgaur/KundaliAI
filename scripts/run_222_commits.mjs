...
import { batch1 } from './commits_222_batch1.mjs';
import { batch2 } from './commits_222_batch2.mjs';
import { batch3 } from './commits_222_batch3.mjs';
import { batch4 } from './commits_222_batch4.mjs';
import { batch5 } from './commits_222_batch5.mjs';
import { batch6 } from './commits_222_batch6.mjs';
...
console.log(`Loaded ${allCommits.length} commits for the 222 suite.`);
...
function executeCommitStep(item) {
  ...
  try {
    ...
    console.log(`[${step}/222] OK: ${msg}`);
  } catch (err) {
    fs.appendFileSync(path.join(rootDir, 'streak.txt'), `\n# [${step}/222] ${msg}\n`, 'utf-8');
    execSync('git add -A', { cwd: rootDir, stdio: 'pipe' });
    const escapedMsg = msg.replace(/"/g, '\\"');
    execSync(`git commit -m "${escapedMsg}"`, { cwd: rootDir, stdio: 'pipe' });
    console.log(`[${step}/222] OK (touched): ${msg}`);
  }
}

console.log('🚀 Starting sequence of 222 commits...');
for (const item of allCommits) {
  executeCommitStep(item);
}

console.log('\n🎉 ALL 222 COMMITS CREATED SUCCESSFULLY!');
...
try {
  ...
  console.log('🚀 Successfully pushed 222 commits to GitHub!');
} catch (pushErr) {
